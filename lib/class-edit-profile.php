...
if ( ! class_exists( 'em_edit_profile' ) ) :
...
class EM_Edit_Profile {
	...
	public function edit_profile() {
		 ...
		 * @since 1.0.0
		 */
		add_shortcode( 'em_edit_profile', function () {
			 ...
			 * @since 1.0.0
			 */
			if ( file_exists( EM_DIR . '/lib/view-edit-profile.php' ) ) {
			    require_once( EM_DIR . '/lib/view-edit-profile.php' );
			}
	...
	public function update_profile() {
		...
		// Verify the nonce.
        if( wp_verify_nonce( $_POST['em_update_profile_nonce'], 'em_update_profile' ) ) {
            ...
            if ( ! empty( $_POST['email'] ) ){
                ...
                if ( ! $user_email )
                    $errors[] = __( 'Provided email address is invalid.', 'EM' );
                else {
                    // email_exists returns a user id if a user exists against it.
                    $email_exists = email_exists( $user_email );
                    if( $email_exists ) {
                        if( $email_exists != $user_id ){
                            $errors[] = __('Provided email is already in use by another user. Try a different one.', 'EM');
                        } else {
            ...
            if ( ! empty($_POST['pass1'] ) && ! empty( $_POST['pass2'] ) ) {
                if ( $_POST['pass1'] == $_POST['pass2'] ) {
                    ...
                    $errors[] = __('The passwords you entered do not match.  Your password is not updated.', 'EM');
                }
            }
            ...
            if ( count( $errors ) == 0 ) {
                ...
                $response = array(
                    'success' => true,
                    'message' => __( 'Submitted! <br/> Your profile information was successfully updated!', 'EM' ),
                );
            ...
            $errors[] = __('Security check failed!', 'EM');
        } // if nonce ended.
